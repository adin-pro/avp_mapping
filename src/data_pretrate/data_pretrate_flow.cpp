...
DataPretreatFlow::DataPretreatFlow(ros::NodeHandle &nh) {
  ...
  odom_pub_ptr_ = std::make_shared<OdometryPublisher>(
      nh, "/synced_odom", "/map", "/base_link", 1000);
...
bool DataPretreatFlow::readData() {
  ...
  // LOG(INFO) << "Unsynced Odom buffer size: " <<
  // unsynced_odom_data_buff.size();
  ...
  if (odom_data_buff_.size() > 0) {
    LOG(INFO) << "Odom front before sync: " << odom_data_buff_.front().time;
  }
  bool sync_odom =
      PoseData::syncDataWithoutInterpolation(odom_data_buff_, img_sync_time_);
  if (odom_data_buff_.size() > 0) {
    LOG(INFO) << "Odom front after sync: " << odom_data_buff_.front().time;
  }
...
bool DataPretreatFlow::publishData() {
  ...
  if (odom_data_buff_.size() > 0) {
    odom_pub_ptr_->publish(odom_data_buff_.front().pose,
                           odom_data_buff_.front().time);
